...
def hello_world(request):
    if request.user.is_authenticated:  # 유저 접속 확인
        ...
        return HttpResponseRedirect(reverse('accountap:login'))  # 로그인 안되면 로그인 창으로 돌려보냄
...
class AccountUpdateView(UpdateView):  # 업데이트 창
    ...
    def get(self, *args, **kwargs):  # 망치는 과정 : 어카운트 로그인없이 페이지 수정 방지
        if self.request.user.is_authenticated and self.get_object() == self.request.user:   #로그인한 사람이 정보를 바꾸려는 유저랑 같은지 확인
            return super().get(*args, **kwargs)
        else:
            return HttpResponseForbidden()

    def post(self, request, *args, **kwargs):  # post도
...
class AccountDeleteView(DeleteView):  # 회원탈퇴
    ...
    def get(self, *args, **kwargs):  # 망치는 과정 : 어카운트 로그인없이 페이지 수정 방지
        if self.request.user.is_authenticated and self.get_object() == self.request.user:
            ...
            return HttpResponseForbidden()  #금지된 곳 접근
